...
**IT capacity**: The IT team reclaims an estimated 3 to 5 business days currently spent on manual provisioning, freeing them for higher-value work.
...
**Edge cases still need humans**: Unusual situations — like a hire into a brand-new department, an international transfer with region-specific access needs, or a sensitive role — will be routed to IT for manual review. The system is designed to handle the 90% of onboardings that are straightforward and escalate the 10% that aren't.
...
**Human oversight is built in.** The system doesn't operate as a black box. Every action it takes is logged and explainable. For sensitive roles, it explicitly waits for human approval before proceeding (In the role_policy.json, Finance and InfoSec currently have "requires_approval: true" set). IT retains full control and can pause, override, or roll back any onboarding at any time, for example "rollback_user" and "check_provisioning_status" are exposed as MCP tools, an IT admin using Claude Desktop or a Slack bot could call them manually at any time.
...
**This amplifies IT capabilities** The team still owns the onboarding process, the access policies, and the exception handling. The system handles the repetitive execution so they can focus on the decisions that actually require human judgment.
